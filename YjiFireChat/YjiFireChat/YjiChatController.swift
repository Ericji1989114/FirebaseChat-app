...
class YjiChatController: JSQMessagesViewController {
    // param.
    ...
    override func viewDidLoad() {
        ...
        var userId = ""
        var userName = ""
        if let id = UserDefaults.standard.object(forKey: UserInfoKey.userId) as? String {
            userId = id
        }
        if let name = UserDefaults.standard.object(forKey: UserInfoKey.userName) as? String {
            userName = name
        }
    ...
    override func didPressSend(_ button: UIButton, withMessageText text: String, senderId: String, senderDisplayName: String, date: Date) {
        ...
        guard let message = YjiMsg(messageId: messageId, senderId: senderId, senderDisplayName: senderDisplayName, date: date, text: text) else {return}
    ...
    private func sendMessageToFirebase(message: YjiMsg) {
        let timeStr = YjiComonFunc.getDateFormatter().string(from: message.date)
    ...
    private func showMessages(resultDic: [(key: String, value: Any)]) {
        for resultInfo in resultDic {
            ...
            let dateTime = YjiComonFunc.getDateFormatter().date(from: timeStr)
            let messageIdStr = "\(messageId)"
            
            guard let message = YjiMsg(messageId: messageIdStr, senderId: userId, senderDisplayName: userName, date: dateTime, text: content) else {continue}
    ...
    private func addNewMessage(resultDic : [(key: String, value : Any)]) {
        for resultInfo in resultDic {
            ...
            if (!hasMsg) {
                guard let message = YjiMsg(messageId: messageIdStr
                    , senderId: userId
    ...
    func addMedia(_ media:JSQMediaItem) {
        let messageId = YjiComonFunc.getMessageId()
        
        guard let message = YjiMsg(messageId: messageId, senderId: senderId, senderDisplayName: self.senderDisplayName, date: Date(), media: media) else {return}
        self.messages.append(message)
        self.finishSendingMessage(animated: true)
    }
    ...
    override func collectionView(_ collectionView: JSQMessagesCollectionView, avatarImageDataForItemAt indexPath: IndexPath) -> JSQMessageAvatarImageDataSource? {
        ...
        return YjiComonFunc.getIconImage(id: message.senderId)
    }
